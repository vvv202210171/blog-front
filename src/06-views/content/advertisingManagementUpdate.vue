<template>
  ...
  <script>
  import { getArticleTypeList, getAdvertiseById, mergeAdvertise } from "@/03-api/article";
  ...
  export default {
    ...
    methods: {
      async loadAdvertiseList() {
        this.loading = true;
        const Category = await getArticleTypeList();
        this.cates = this.buildTree(Category);
        this.form.Status = this.checkStatus[0].val;
        const Id = this.$route.query.Id;
        const data = await getAdvertiseById({ Id });
        for (const [key, value] of Object.entries(data)) {
          this.form[key] = value;
        }
        this.form.Image = this.form.ImageUrl;
        this.publishDate = [this.form.DisplayFrom, this.form.DisplayTo];
      ...
      async onSubmit() {
        try {
          ...
          if (await this.$refs.form.validate()) {
            ...
            param.Image = this.form.update_image ? this.file : "";
