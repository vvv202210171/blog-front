  ...
  <div class="app-container pay-channel-setting">
    ...
    <Report v-loading="loading" :list="list" width="100%" :expand="conf.report.expand" :items="conf.report.items" :actions="conf.report.actions" @row-action="RowAction($event)">
      ...
      <template #MaxServiceFeeAmount="{ item }">
        <div :class="['el-form-item', { 'is-error': item.MinServiceFeeAmount > item.MaxServiceFeeAmount } ]">
...
<script>
import { getCashoutSetting, updateCashoutSetting } from '@/03-api/merchant'
import conf, { search } from './config/cashoutSetting'
...
export default {
  ...
  destroyed() {
    this.$store.commit('merchant/REMOVE_ACTIVE')
  },
  methods: {
    async loadList() {
      try {
        ...
        this.loading = true
        const data = await getCashoutSetting(this.params)
        this.list = data
        this.loading = false
      } catch (error) {
        this.loading = false
    ...
    changeMerchantStatus(row) {
      const status = row.status !== 0
      const confirm = this.Confirm(this.$t('merchant_0003').replace(/\{\s?status\s?\}/, status ? this.$t('common_0048') : this.$t('common_0049')))
      confirm(updateCashoutSetting, { merId: row.merId, disableOrEnable: status })
    },
    ...
    async save() {
      ...
      this.submiting = true
      const { code, msg } = await updateCashoutSetting({ MerchantAlias: this.active.MerchantAlias, SettingList: this.list }, { isAction: true })
