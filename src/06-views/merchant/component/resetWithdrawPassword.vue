  ...
  <el-dialog class="reset-password" :title="$t('report_0262')" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
    <el-form ref="form" :rules="rules" :model="form" label-position="top" label-width="100px">
      <el-form-item class="" :label="$t('report_0038')" prop="MerchantAlias">
        <el-input v-model="form.MerchantAlias" readonly />
      </el-form-item>
      <el-form-item class="" :label="$t('report_0039')" prop="MerchantName">
        <el-input v-model="model.MerchantName" readonly />
      </el-form-item>
      <el-form-item :label="$t('report_0256')" prop="NewWithdrawPassword">
...
<script>
import { resetWithdrawPassword } from '@/03-api/merchant'
  ...
  watch: {
    show(value) {
      if (value) {
        ...
        this.form.MerchantAlias = this.model.MerchantAlias
        this.$nextTick(() => {
    ...
    async confirm() {
      try {
        if (await this.$refs.form.validate()) {
          ...
          const { code, msg } = await resetWithdrawPassword(param, { isAction: true })
          this.loading = false
          if (code === 0) {
