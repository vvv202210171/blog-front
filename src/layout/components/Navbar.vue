<template>
  <div class="navbar">
    ...
    >
      ...
      <span
        v-if="hasRoleWithdraw"
        style="padding: 10px; border-radius: 30px"
        :class="
          total.cashoutOrderRequestCount > 0 ? 'spanRequest' : 'spanRequest1'
        "
        ><a @click="withdrawUrl()"
          >{{ $t("report_0365") }}{{ total.cashoutOrderRequestCount }}</a
        ></span
      >
      <a
        ><span
          ...
          :class="
            showMusic
              ? 'el-icon-message-solid spanVoice-open'
              : 'el-icon-bell spanVoice-close'
          "
          @click="music"
        >
          {{ showMusic ? $t("report_0366") : $t("report_0367") }}</span
        ></a
      >
    ...
    <audio
      id="audio2"
      src="./Music/member_withdrawal_default.mp3"
      class="audio2"
      preload="auto"
      :muted="!showMusic"
    />
